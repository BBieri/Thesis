...
Counts and the share of environmental patents. We estimate the first two
...
[@papkeEconometricMethodsFractional1996] and complements the analysis of the
*WPH* by linking environmental regulation and environmental innovation.
...
stringency on R&D investments and on patent counts in either of the two
backbone extraction specifications.
$Strength_{i,\ t-1}$, our first international policy variable,
...
within the environmental cooperation network also shows consistent positive point
estimates in the second and fifth model investigating patents. The result in the
...
observation in these model is that the international policy variables supplant
...
of either international policy variable. Additionally the point estimates are
not consistent across backbone extraction procedures and standard errors are
...
are systematically bigger in the FDSM models for our international policy
...
cannot however be interpreted as the previous double log models due to their
...
effect on the share of environmental patents.
We also observe a large positive effect of GDP per capita under all specifications
...
countries.
Curiously, @martinez-zarzosoDoesEnvironmentalPolicy2019 do not control for
...
and may thus lead to different conclusions on the impact of import and export
...
\end{adjustbox}
\par \raggedright 
\footnotesize{Newey West corrected standard errors in parentheses. * significant 10\%, ** significant 5\%, *** significant 1\%.
Within $R^2$ and overall F-Test are not availiable for the QMLE estimation due to software limitations.}
\par\endgroup
```
...
subsection, we subset our analysis in two parts. We first consider the
models describing the *WPH* before turning our attention to the complementary
environmental patent share regressions.
...
the $95\%$ confidence interval of our effect. A second cause for this inverse
...
not our patent count specification.
...
in the same lag structure which may not be the case.
...
expenditures as well as patent counts. Crucially in this instance is that
...
the innovation output model (patents).].
...
none of the policy coefficients are significant, we cannot conclude
that five year lagged environmental policy increased the share of environmental
...
positive effect across all specifications and whose point estimates are larger
...
investments which are already inherently risky. And finally, the third observation
...
\par \raggedright 
\footnotesize{Newey West corrected standard errors in parentheses. * significant 10\%, ** significant 5\%, *** significant 1\%.
Within $R^2$ is not availiable for the QMLE estimation due to software limitations.}
\par\endgroup
```
...
in the same lag and backbone extraction structures. Since the dependent variable
...
effect of a $1\%$ change in growth rate percentage points.
...
with care as they are relatively large. While they cannot be directly compared to the
results @martinez-zarzosoDoesEnvironmentalPolicy2019 found due to the fact that
their results considered an old *level* index of TFP of the EU KLEMS dataset
...
fixed effects to account for these.].
...
are not contingent on its choice as there exist no precise theoretical foundation
that would motivate the choice of one backbone extraction method over another.
This leads us to caution against the use of a *single* backbone extraction process
to correct our bipartite networks as its choice may have consequences on the
...
More standard econometric checks were performed as well.
...
states whether domestic or international.
...
patent counts^[Results are presented in the appendix.].
