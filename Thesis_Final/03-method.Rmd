...
This measures the *input* of the R&D process. $TPF_{it}$ is the count
...
policy stringency and/or our international policy measures or not.
...
more detail the $\text{EPS}_{it}$ indicator as well as the various other dependent
...
The network is depicted in \@ref(fig:ECOLEXbipartitenetwork)
...
```{r ECOLEXbipartitenetwork, echo=FALSE, fig.pos = 'h', fig.cap = "Bipartite Membership Network Snapshots"}
...
of bipartite networks are chiefly responsive for the structure of its
...
still able to estimate the backbone via this method within a day.].
...
the set of simulated networks satisfy this constraint only on the *average*
...
the average condition [@saraccoInferringMonopartiteProjections2017]. Finally,
we note that given our bipartite network characteristics and the computational
...
We will therefore compute the centrality scores on the basis of the latter two
corrected networks.

```{r ComparingBackbones, fig.cap="Comparing Backbone Extraction Methods"}
...
international environmental governance complex which we hypothesize affect in
direct and indirect ways the rates of innovation and/or its competitiveness.
While there exist a multitude of centrality indices to characterize the
...
transitivity or triadic closure measure which we will theoretically describe
in turn.
...
it measures the quantity of agreement a country is bound by. This measure can thus
...
On the illustration below \@ref(fig:transitivitygraph), we see that
...
from the weights, it is easy to see that the transitivity coefficient
...
of the actions undertaken by each other node over time [@colemanSocialCapitalCreation1988].
...
This measure therefore proxies the *indirect effects* of international embeddedness
on our dependent variables.

Now that we have defined the computation of a novel set of measures characterising
...
```{r BERDdesc, echo=FALSE, fig.cap="Business Expenditures in R\\&D 1990-2015", fig.pos="h", warning=FALSE, message=FALSE}
readRDS(paste0(clean_data_path, "Panel.rds")) %>%
...
```{r TPFdesc, echo=FALSE, fig.cap="Triadic Patent Counts 1990-2015", fig.pos="h", warning=FALSE, message=FALSE}
readRDS(paste0(clean_data_path, "Panel.rds")) %>%
...
```{r EnvPatdesc, echo=FALSE, fig.cap="Environmental Patent Share 1990-2015", fig.pos="h", warning=FALSE, message=FALSE}
readRDS(paste0(clean_data_path, "Panel.rds")) %>%
...
period around $0\%$ to $2.5\%$ where high-income countries tend to have a
...
years, the TFP growth rate recovered rapidly. Finally, note that due to data availability
we estimate this model from 1996 onward only.

```{r TFP0desc, echo=FALSE, fig.cap="Total Factor Productivity 1996-2015", warning=FALSE, message=FALSE}
readRDS(paste0(clean_data_path, "Panel.rds")) %>%
...
weakly negatively correlated to our policy measures which seemingly hints at the fact that
