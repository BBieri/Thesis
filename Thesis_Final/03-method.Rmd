...
$e$.] Following  therefore leverage the False Discovery Rate multiple test correction
...
examining the centrality of a node. In mathematical terms, the
eigenvector centrality of a node in a weighted and undirected network is
defined as the following index: 
$$
c_{i}^{w}=\frac{1}{s_{i}\left(k_{i}-1\right)} \sum_{j, h} \frac{\left(w_{i j}+w_{i h}\right)}{2} a_{i j} a_{i h} a_{j h}
$$
In our international cooperation
network, this can be trans
